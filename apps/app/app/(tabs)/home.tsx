...
export default function Home() {
  ...
  const { user, prefs, filters, setFilters, isFavCourt, toggleFavCourt } = useAppState();

  const courts = useMemo(() => getCourtListItems(), []);
  const popular = useMemo(() => sortCourts(courts, 'rating').slice(0, 5), [courts]);
    ...
    <ScreenContainer>
      <View style={styles.body}>
        <View style={styles.greeting}>
          ...
          <Text style={textStyle('h1App', 'ink')}>Ciao {user?.name ?? 'Atleta'} 👋</Text>
        </View>
        ...
        >
          <SportChip label="Tutti" active={filters.sport === 'all'} onPress={() => quickSport('all')} />
          {prefs.sports.map((id) => (
            <SportChip key={id} label={sportLabel(id)} active={filters.sport === id} onPress={() => quickSport(id)} />
          ))}
        </ScrollView>

        <View style={styles.section}>
          <SectionHeaderRow title="Campi popolari" onAction={() => router.push('/search')} />
        ...
        <View style={styles.section}>
          <SectionHeaderRow title="Consigliati per te" onAction={() => router.push('/search')} />
        ...
        <View style={styles.section}>
          <SectionHeaderRow title="Intorno a te" onAction={() => router.push('/search')} />
...
const styles = StyleSheet.create({
  ...
  carousel: { gap: theme.spacing.md, paddingVertical: theme.spacing.xs, paddingRight: theme.spacing.lg },
