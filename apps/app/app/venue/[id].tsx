    ...
    <View style={styles.root}>
      <ScrollView
        ...
        contentContainerStyle={{
          paddingBottom:
            insets.bottom + theme.layout.ctaHeight + theme.spacing.xxxl,
        }}
      >
        {/* Hero */}
        <CourtHero
        ...
        <View style={styles.bodyPad}>
          <Card style={styles.detailCard}>
            <View style={styles.detailTop}>
              <SportTag
                label={sportLabel(venue.sportIds[0] ?? "")}
                sportId={venue.sportIds[0] ?? "padel"}
              />
              <AvailabilityBadge
                state={venue.open ? "open" : "closed"}
                label={venue.open ? "Aperto ora" : "Chiuso"}
              />
            </View>
...
function InfoTab({
  ...
  venue: NonNullable<ReturnType<typeof getVenueById>>;
  courtCount: number;
}) {
  return (
    <View style={styles.section}>
      ...
      <View style={styles.statsRow}>
        ...
        <DetailStat
          ...
          value={venue.distance}
          label="Distanza"
        />
        <DetailStat
          icon={venue.indoor ? "home-outline" : "sunny-outline"}
          value={venue.indoor ? "Indoor" : "Outdoor"}
          label="Tipo"
        />
        <DetailStat
          icon="star-outline"
          value={formatRating(venue.rating)}
          label="Rating"
        />
      </View>
    ...
    <View style={styles.section}>
      <Card style={styles.reviewSummary}>
        ...
        <View style={{ gap: 2 }}>
          ...
          <Text style={textStyle("caption", "muted")}>
            {pluralize(count, "recensione", "recensioni")}
          </Text>
        </View>
      </Card>
