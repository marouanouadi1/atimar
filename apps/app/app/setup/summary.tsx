...
export default function Summary() {
  ...
  const availability =
    `${prefs.availability.days.join(', ') || '—'} · ${prefs.availability.times.map(timeLabel).join(', ') || '—'}`;
    ...
    >
      <View style={styles.body}>
        <View style={styles.hero}>
          <CheckBadge />
          <ScreenTitle title="Tutto pronto!" subtitle="Abbiamo personalizzato ATIMAR in base alle tue preferenze." size="h1" align="center" />
        </View>

        <View style={styles.rows}>
          <SummaryRow icon="tennisball-outline" label="Sport" value={sports} />
          <SummaryRow icon="location-outline" label="Zona" value={zone} />
          <SummaryRow icon="time-outline" label="Disponibilità" value={availability} />
        </View>
      </View>
    </ScreenContainer>
  );
