...
import { getSupabaseClient } from '@atimar/api';
import Link from 'next/link';

export default async function DashboardPage() {
  const supabase = getSupabaseClient();
  const [{ count: strutture }, { count: campi }, { count: utenti }] =
    await Promise.all([
  ...
  const stats = [
    {
      ...
      cardColor: 'bg-gradient-to-br from-blue-400 to-blue-600 text-white',
    ...
    },
    {
      ...
      cardColor: 'bg-gradient-to-br from-emerald-400 to-emerald-600 text-white',
    ...
    },
    {
      ...
      cardColor: 'bg-gradient-to-br from-violet-400 to-violet-600 text-white',
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {stats.map(({ label, value, Icon, iconColor, cardColor, href }) => (
            <Link key={label} href={href}>
              <Card
                className={`overflow-hidden hover:opacity-90 transition-opacity cursor-pointer border-0 ${cardColor}`}
              >
